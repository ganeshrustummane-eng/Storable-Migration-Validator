...
- The only live execution engine that consumes the generated YAML: `Project/main.py` (row-level pass/fail, called via `Project/runner.py` by the webapp's "Run Validation" button, layer-agnostic — bronze and silver both flow through it). `src/validation/validation_executor.py` was the chat-agent-only engine (used by the now-removed `execute_validation` tool) — it has no caller left and was moved to `trash/validation/`; don't generate for it.
- **Silver layer (Snowflake-to-Snowflake) shares this schema/generator layer but has its own upstream extractor.** `src/silver/coalesce_plan_builder.py` (owned by the `silver-layer-coalesce-specialist` agent) turns Coalesce node metadata into a `CanonicalValidationPlan` and hands it to the same `generate_from_plan()`/`write_from_plan()` you own — you don't touch Coalesce parsing, but if Silver work needs a new field on `CanonicalValidationPlan`/`ColumnMappingEntry` or a generator change, that edit is yours; hand the exact field/contract back to the Silver specialist rather than it editing `validation_plan.py` itself.
