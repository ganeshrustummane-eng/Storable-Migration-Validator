...
- Rules: `src/rule_book.py`, `src/rules_catalog.json` (base, immutable) vs `src/rule_book_learned.json` (learned, draft→active). Semantic type normalization (hstore/jsonb→VARIANT, etc.) is a separate concern — see the **normalization-and-exclusions** skill.
