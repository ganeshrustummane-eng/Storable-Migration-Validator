...
- `src/silver/coalesce_plan_builder.py` — `build_plan(node_id, workspace_id=None) -> (CanonicalValidationPlan, SchemaDiff)`. The 4-bucket column classifier, cross-node `columnReferences` resolution, deferred surrogate-key handling, and `write_schema_diff_exclusion()`. This is the one real net-new logic per ADR 0014 §3/§4/§5 — most future Silver work lands here.
- `src/silver/test_coalesce_plan_builder.py` — assert-based self-check fixture shaped like the sample `INT_FACILITIES` node. Extend this, don't replace it, when adding a new classification case or a second real sample node.
...
- Downstream, **shared with Bronze, not owned by you**: `src/core/validation_plan.py` (`CanonicalValidationPlan`/`ColumnMappingEntry` schema), `src/generated_queries/sql_query_generator.py`/`ai_sql_generator.py`/`yaml_config_writer.py`, `Project/main.py --layer_type silver`. If Silver work needs a new field on the shared plan schema or generator, that edit belongs to `validation-query-yaml-generator` — hand it the exact field/contract needed rather than editing those files yourself, so the schema stays one source of truth for both layers.
- UI call sites live in `webapp/app.py`'s Silver sub-flow (Bronze/Silver radio in `tab_batch`) — that belongs to `streamlit-frontend-manager`. Expose a function/return shape; don't edit `app.py`.

## Known gaps to be aware of (don't silently "fix" without flagging — these are documented, deliberate deferrals or open items)

- **No batch/multi-node loop yet.** Today's flow is one Coalesce node at a time (`build_plan()` called once per UI interaction). Extending to "n tables across n databases" needs a node-ID list input, a per-node loop, and possibly a new `coalesce_client` "list nodes in workspace" endpoint — real new work, not a bug fix.
- **Bronze-side schema assumption is unconfirmed.** `coalesce_plan_builder.py` currently reuses the Silver node's own schema name for the Bronze side (ADR 0014 §2 flags this as unverified against a second sample). Confirm or fix this before trusting it across a table that crosses schema boundaries.
- **No macro/Jinja interpreter, by design.** Macro-computed (surrogate key) columns are `skip_validation=True`, never hand-reimplemented (ADR 0014 §3/§6). If a specific macro needs validating, that's a new narrowly-scoped ADR for that macro, not a general interpreter.
- **Recomputable-expression columns (bucket 2) store raw SQL in `source_column`.** Existing type-normalization rules assume a bare identifier; non-trivial expressions may be mis-wrapped. Flagged in the skill file as future work — don't quietly patch the shared rule code to special-case this without raising it first.
